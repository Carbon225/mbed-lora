...
LoRaClass::LoRaClass(PinName mosi, PinName miso, PinName sck, PinName ss, PinName rst, PinName dio0) :
    _spi(mosi, miso, sck),
    _ss(ss, 1), _reset(rst, 1), _dio0(dio0),
    _frequency(0),
    _packetIndex(0),
...
void LoRaClass::onReceive(void(*cb)(int))
{
    _onReceive = cb;
    ...
    if (cb) {
        ...
        _dio0.rise(callback(this, &LoRaClass::handleDio0Rise));
    } else {
        _dio0.disable_irq();
    }
}
